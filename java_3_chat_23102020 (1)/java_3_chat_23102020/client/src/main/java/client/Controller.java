...
public class Controller implements Initializable {
    ...
    private static String logFileName (String nickname) {
        String fileName = "log/history_" + nickname + ".txt";
        return fileName;

    }
    private static void wrtToLogFile (String msg, String nickname) {

        try (FileWriter out = new FileWriter(logFileName(nickname), true)) {
    ...
    private static String  readToLogFile (String nickname) {
        StringBuilder sb = new StringBuilder();
        try {
            List<String> logList = Files.readAllLines(Paths.get(logFileName(nickname)));
            int logstart = 0;
            if (logList.size() > 100) {
                logstart = logList.size() - 100;
            }
            for (int i = logstart; i < logList.size(); i++) {
                sb.append(logList.get(i)).append(System.lineSeparator());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
